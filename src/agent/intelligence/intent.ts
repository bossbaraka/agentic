...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    patterns: [
      /(?:شو المزايا|ايش المزايا|مزايا|الميزات|ميزات|وش تحصل|شو يشمل|شو تشمل|ايش تشمل|شو تتضمن|شو الخدمات اللي|هل فيه|هل يدعم المنيو|هل يدعم شاشه|الشاشه|شاشه المطبخ|kds|pos|كاشير|منيو qr|استدعاء النادل|نداء النادل|هل احتاج|هل يمكنني|هل يقدر|هل تقدروا|تقييم|تقارير|تحليلات|شعار|هويه|ترقيه|الغاء|اشتراك سنوي|توفير سنوي|خصم|فواتير|صلاحيات|pin)/i,
      /features|include|what do i get|upgrade|discount|report|dashboard|kitchen screen|qr menu/i,
    ],
  },
  {
    intent: 'product_information', weight: 6,
    patterns: [
      /(?:شو عندكم|ايش عندكم|وش عندكم|شو هو مريح|شو هي مريح|عرفني|احكيلي عن|ممكن تشرح|اشرح لي|شو حلولكم|خدماتكم|شو بتقدموا|بماذا تساعدون|شو قصتكم|من منتجاتكم|منصه مريح|منصة مريح)/i,
      /what do you (?:offer|have)|tell me about|your services|what is mureeh/i,
    ],
  },
  {
    intent: 'service_information', weight: 7,
    patterns: [
      /(?:موقع الكتروني|موقع لمطعمي|مواقع|تصميم موقع|بدي موقع|ابي موقع|ابغى موقع|اريد موقع|وكيل ذكاء|ذكاء اصطناعي|بوت واتساب|وكيل واتساب|واتساب ذكي|اداره (?:سوشال|التواصل|انستغرام|الانستا)|نظام حجوزات|حلول مخصصه|خدمات رقميه|الخدمات الرقميه|سوشال ميديا|تيك توك)/i,
      /website design|digital services|custom (?:solution|development)|whatsapp (?:bot|agent)|ai agent|social media management|booking system/i,
    ],
  },
  {
...
const REAL_QUESTION = /(?:سعر|اسعار|بكم|قديش|باقه|باقات|طاول|اشترك|تفعيل|حجز|موعد|مشكله|دعم|كيف (?:بدي|ابغى|ارد|اقدر|احصل|احجز|اشترك|اشتراك|افعل)|شو الفرق|انصح|فرع|منيو|كاشير|مطبخ|موقع|واتساب|سوشال)/i; // مطبَّع
